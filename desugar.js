...
var desugarAST = (function() {
    ...
    "while": 
        "lambda() {" +
            "def %u2 (condition, body) { " + 
                ...
                "};" +
            "};" +
            "%u2(lambda() { %condition }, lambda() { %body });" +
        "}()",
    "for": 
        "lambda() {" +
            "def %u4 = 0;" + 
  ...
  function desugar(node) {
    ...
    // leaf node
    } else if (node.type == 'call' && node.function && node.function.name == 'name') {
      return desugar({
        'type': 'type',
        'arguments': node.arguments
      });
    } else if (node.type == 'call' && node.function && node.function.name == 'draw') {
      return desugar({
        'type': 'draw',
        'arguments': node.arguments
      });
    ...
    } else if (node.type == 'call' && node.function.name && node.function.name == 'type') {
      var res = {'type': 'type',
                 'arguments': node.arguments
      };
      return res;
